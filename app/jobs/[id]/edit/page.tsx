...
export default function EditJobPage() {
  ...
  useEffect(() => {
    const loadJob = async () => {
      ...
      setLoadError(null)
      try {
        const job = await apiClient.jobs.get(params.id)
        if (!job) {
          setLoadError('Job profile not found.')
          return
        }
        const populated: JobProfileFormData = {
          title: job.title,
          department: job.department,
          location: job.location,
          openings: job.openings,
          description: job.description ?? '',
          mandatory: job.mandatoryCriteria,
          preferred: job.preferredCriteria,
          weights: job.scoringWeights,
        }
        setFormData(populated)
      } catch {
        setLoadError('Failed to load job profile.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSave = async (status: 'active' | 'draft') => {
    setSaving(true)
    try {
      await apiClient.jobs.update(params.id, {
        ...formData,
        status,
      ...
      toast.success(status === 'active' ? 'Job profile updated and activated!' : 'Changes saved as draft')
      router.push(`/jobs/${params.id}/results`)
    } catch {
    ...
    return (
      <div className="min-h-screen" style={{ background: '#0A0A0F' }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="h-8 w-56 rounded mb-8 animate-pulse" style={{ background: '#1E1E2E' }} />
          <div className="flex gap-8">
            <div className="hidden lg:flex flex-col gap-2 w-56 flex-shrink-0">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="h-14 rounded-xl animate-pulse" style={{ background: '#1E1E2E' }} />
              ))}
            </div>
            <div className="flex-1">
              <div className="rounded-xl animate-pulse" style={{ background: '#111118', border: '1px solid #1E1E2E', minHeight: 500 }} />
            </div>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#0A0A0F' }}>
        <div className="text-center">
          <p className="text-sm mb-4" style={{ color: '#EF4444', fontFamily: 'DM Sans, sans-serif' }}>{loadError}</p>
          <Link href="/"
